...
void quit(const Arg *arg) {
    ...
    // 杀死 autostart 启动的整个进程组
    if (autostart_pgid > 0) {
        kill(-autostart_pgid, SIGTERM); // 使用负号发送信号给整个进程组
        kill(autostart_pgid, SIGKILL);  // 确保彻底杀死（可选）
    }
...
void runAutostart(void) {
    ...
    if (autostartscript == NULL || strlen(autostartscript) == 0)
        return;

    snprintf(cmd, sizeof(cmd), "%s", autostartscript);

    if ((pid = fork()) == 0) {
        if (setsid() < 0) // 创建新会话并成为组长
            exit(EXIT_FAILURE);
        autostart_pgid = getpgrp(); // 记录进程组ID
        execl("/bin/sh", "sh", "-c", cmd, NULL);
        exit(EXIT_SUCCESS);
    } else if (pid > 0) {
        autostart_pgid = pid; // 也可以用 waitpid 或直接记录 pid
    }
}
